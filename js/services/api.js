		...
		objects.forEach(function(obj) {
			if(obj.appName && obj.objectId) {
				...
				if(appObject) {
                    ...
                    var promise = appObject.app.getObject('qso_' + obj.objectId, obj.objectId, obj.objectParam);
		...
		angular.forEach($rootScope.global.apps, function(value, key) {
            if(value.model.length > 0) {
				for (var i = 0; i < value.model.length; i++) {
                    if (!$rootScope.qlikRootObjects || ($rootScope.app && $rootScope.qlikRootObjects[$rootScope.app] && $rootScope.qlikRootObjects[$rootScope.app].indexOf(value.model[i].id) === -1)) {
